...
class ActorNetwork(nn.Module):
    def __init__(self, mlp_input_dim, image_channels, actor_hidden_dims, num_actions, activation=nn.ReLU(), use_vis=True, options=None):
        super(ActorNetwork, self).__init__()

        self.options = options
        # If no vision and no refiner, use pure MLP actor
        if not use_vis:
            ...
            # CNN と MLP の結合後のネットワーク (Actor Head)
    ...
    def forward(self, obs):
        ...
        if self.options is not None and "command_refiner" in self.options:
            ...
            command_refiner_input_image_features = command_refiner_input_image_features.view(command_refiner_input_image_features.size(0), -1)
            ...
            # print(f"before command_refiner (actor): {mlp_obs[0, 6:9]}")
            # print(f"before command_refiner: {mlp_obs[0, :]}")
            mlp_obs = mlp_obs.clone()
            mlp_obs[:, 6:9] = refined_commands_scaled
            # print(f"after command_refiner (actor): {mlp_obs[0, 6:9]}")
...
if __name__ == "__main__":
    ...
    use_vis = False
    image_channels = 1
    model_mlp = ActorNetwork(mlp_input_dim, image_channels, actor_hidden_dims, num_actions, use_vis=use_vis, options=options)
    ...
    # vision example
    image_size = 64
    model_vis = ActorNetwork(mlp_input_dim, image_channels, actor_hidden_dims, num_actions, use_vis=use_vis, options=options)
